...
/// </summary>
public class GridAlgorithm : LayoutManager
{
    ...
    public GridAlgorithm(Microsoft.Maui.ILayout layout) : base(layout)
    {
    }
    ...
    {
        this.GetChildTable(bounds.Width, bounds.Height);
        ...
        for (int rowIndex = 0; rowIndex < table.RowInfos.Count; rowIndex++)
        {
            ...
            for (int columnIndex = 0; columnIndex < table.ColumnInfos.Count; columnIndex++)
            {
                ...
                {
                    childInfo.Child.Arrange(new Rectangle(currentX, currentY, this.GetChildAvailableWidth(childInfo), this.GetChildAvailableHeight(childInfo)));
                }
        ...
        for (int column = 0; column < table.ColumnInfos.Count; column++)
        {
            ...
            else
            {
                ...
                foreach (var childInfo in columnInfo.ChildInfos)
                {
                    ...
                    var columnWidthForChild = totalSize.Width - otherColumnsWidth;
        ...
        if (starWidthNumber > 0d)
        {
            ...
            if (double.IsPositiveInfinity(width))
            {
                ...
                foreach (var columnInfo in this.table.ColumnInfos)
                {
                    ...
                    foreach (var childInfo in columnInfo.ChildInfos)
                    {
                        if (childInfo.LastStarColumn != columnInfo)
                            continue;
                        ...
                        if (actualAvailableWidth < desiredSize.Width)
                        {
                            ...
                            var neededStarSize = (desiredSize.Width - actualAvailableWidth) / childStarNumber;
                ...
                foreach (var columnInfo in table.ColumnInfos)
                {
                    ...
                    else
                    {
                        foreach (var childInfo in columnInfo.ChildInfos)
                        {
                            ...
                            var impact = size.Width - actualWidthForChild;
        ...
        for (int row = 0; row < table.RowInfos.Count; row++)
        {
            ...
            else
            {
                ...
                foreach (var childInfo in rowInfo.ChildInfos)
                {
                    ...
                    var rowHeightForChild = totalSize.Height - otherRowsHeight;
        ...
        if (starHeightNumber > 0d)
        {
            ...
            if (double.IsPositiveInfinity(height))
            {
                ...
                foreach (var rowInfo in this.table.RowInfos)
                {
                    ...
                    foreach (var childInfo in rowInfo.ChildInfos)
                    {
                        if (childInfo.LastStarRow != rowInfo)
                            continue;
                        ...
                        if (actualAvailableHeight < desiredSize.Height)
                        {
                            var childStarNumber = 0d;
                            for (int i = 0; i < childInfo.RowSpan; i++)
                            {
                            ...
                            var neededStarSize = (desiredSize.Height - actualAvailableHeight) / childStarNumber;
                            starValue = Math.Max(starValue, neededStarSize);
                        }
                    }
                }
                ...
                foreach (var rowInfo in table.RowInfos)
                {
                    ...
                    else
                    {
                        foreach (var childInfo in rowInfo.ChildInfos)
                        {
                            ...
                            var impact = size.Height - actualHeightForChild;
        ...
        public void RegisterChild(IView child)
        {
            ...
            for (int i = 0; i < childInfo.ColumnSpan; i++)
            {
                ...
                else if (columnInfo.ColumnDefinition.Width.IsStar)
                {
                    ...
                    lastStarColumn = columnInfo;
                }
            }

            RowInfo lastAutoRow = null;
            RowInfo lastStarRow = null;
            for (int i = 0; i < childInfo.RowSpan; i++)
            {
            ...
            childInfo.SetLastStar(lastStarColumn, lastStarRow);
    ...
    private class ChildInfo
    {
        ...
        public ChildInfo(IView child, List<ColumnInfo> columns, List<RowInfo> rows)
        {
            this.Child = child;
            this.StartColumnIndex = Grid.GetColumn(child as BindableObject);
            this.StartRowIndex = Grid.GetRow(child as BindableObject);
            this.ColumnSpan = Grid.GetColumnSpan(child as BindableObject);
            this.RowSpan = Grid.GetRowSpan(child as BindableObject);
        ...
        public ColumnInfo LastStarColumn { get; private set; }

        public RowInfo LastStarRow { get; private set; }
        ...
        public Size GetDesiredSize(double width, double height)
        {
            if (desiredSize == null)
                desiredSize = this.Child.Measure(width, height);
        ...
        internal void SetLastStar(ColumnInfo lastStarColumn, RowInfo lastStarRow)
        {
            this.LastStarColumn = lastStarColumn;
            this.LastStarRow = lastStarRow;
        }
    }
}
